...
qemu-system-x86_64 \
    ...
    -drive file=MiniOS11PRO.iso,index=0,media=cdrom \
    -drive file=virtio-win-0.1.229.iso,index=1,media=cdrom \
    -boot menu=on \
    -drive file=Win11.qcow2,if=virtio,index=2,media=disk,aio=native,cache.direct=on,cache=unsafe \
    -display sdl,gl=on \
    ...
    -device virtio-net-pci,netdev=network0,mac=52:54:00:19:38:52 \
    -nic bridge,br=red_interna1,model=virtio-net-pci,mac=52:54:00:44:24:58 \
